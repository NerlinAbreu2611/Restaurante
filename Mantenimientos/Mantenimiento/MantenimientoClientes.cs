...
namespace Mantenimientos
{
    public partial class MantenimientoClientes : Mantenimiento
    {
        ...
        private RepositorioDeCliente repositorio = new RepositorioDeCliente();
        private RepositorioCondicion condiciones = new RepositorioCondicion();
        ...
        private void _CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                Cliente cliente = repositorio.obtenerPorId(id);
                ...
                else if (dataGrid.Columns[e.ColumnIndex].HeaderText == "Estado")
                {
                    ...
                    if (resultado == DialogResult.Yes)
                    {
                        if (cliente.Estado)
                        {
                            repositorio.desahabilitar(cliente);
                        }
                        else
                        {
                            repositorio.habilitar(cliente);
                        }
        ...
        private void cargarDataGrid(List<Cliente> clientes)
        {
            List<Condicion> cond = condiciones.obtenerCondicionesPorEstado(true);
            dataGrid.Rows.Clear();
            ...
            foreach (Cliente c in clientes)
            {
                ...
                foreach (Condicion co in cond)
                {
                    if (c.Id_condicion == co.Id)
                    {
                        condicionDePago = co.Descripcion;
                        Console.WriteLine(condicionDePago);
                        break;
                    }
                }
                ...
                if (c.Estado)
                {
                    dataGrid.Rows.Add(c.Id, c.Nombre, c.Telefono, c.Email, c.Direccion, condicionDePago, Image.FromFile("C:\\Users\\elmen\\Desktop\\imagenes\\pen.png"), Image.FromFile("C:\\Users\\elmen\\Desktop\\imagenes\\eye.png"));
                }
                else
                {
                    dataGrid.Rows.Add(c.Id, c.Nombre, c.Telefono, c.Email, c.Direccion, condicionDePago, Image.FromFile("C:\\Users\\elmen\\Desktop\\imagenes\\pen.png"), Image.FromFile("C:\\Users\\elmen\\Desktop\\imagenes\\hidden.png"));
                }
            }
        ...
        {
            String opcion = Convert.ToString(combo.SelectedItem.ToString());
            if (opcion == "Todo")
            {
                cargarDataGrid(repositorio.ObtenerDatos());
            }
            ...
            {
                cargarDataGrid(repositorio.filtrarInactivos());
            }
        }
        ...
        private void txtBusqueda_TextChanged(object sender, EventArgs e)
        {
            ...
            if (opcion == "Todo")
            {
                cargarDataGrid(repositorio.filtrarPorNombre(txtBusqueda.Text));
            }
            else if (opcion == "Activos")
            {
            ...
            {
                cargarDataGrid(repositorio.filtrarPorNombreInactivo(txtBusqueda.Text));
            }
        }
